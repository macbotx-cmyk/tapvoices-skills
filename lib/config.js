...
function apiBase(config) {
  const base = process.env.TAPVOICES_API_BASE || config.apiBase || DEFAULT_API_BASE;
  return normalizeApiBase(base);
}

function hasToken(config) {
  return Boolean((process.env.TAPVOICES_TOKEN || config.token || '').trim());
}

function requireToken(config) {
  const token = (process.env.TAPVOICES_TOKEN || config.token || '').trim();
  if (!token) {
    const err = new Error(
